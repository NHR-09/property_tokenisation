...
import { api, watchlist, type Property } from "@/lib/api-client"
...
export default function WatchlistPage() {
  ...
  useEffect(() => {
    ...
    api.properties.list().catch(() => []).then(setAllProperties)
  }, [])

  const watchedProperties = allProperties.filter(p => watchedIds.includes(p.id))
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pl-64 transition-all duration-300">
        <DashboardHeader title="Watchlist" subtitle="Properties you're tracking" />
                ...
                {watchedProperties.map((property, index) => (
                  <div key={property.id} className="relative group">
                    ...
                    <Button
                      variant="secondary"
                      size="sm"
                      className="absolute top-3 right-14 opacity-0 group-hover:opacity-100 transition-opacity z-10"
            ...
            <Card className="opacity-0 animate-fade-in-up border-dashed">
              <CardContent className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm text-muted-foreground">Add more properties from the marketplace to compare yields</p>
                </div>
